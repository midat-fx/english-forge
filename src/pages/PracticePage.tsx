...
function ActivationContinuation({ phrase, guided = false }: { phrase: Phrase; guided?: boolean }) {
  ...
  // Заголовок «Тренировочная попытка завершена.» закреплён тестом — не менять.
  const title = guided ? 'Тренировочная попытка завершена.' : stage >= 8 ? 'Слово полностью отработано.' : stage >= 7 ? 'Осталась последняя проверка — завтра.' : stage === 6 ? 'Шаги 1–6 позади.' : lacksReviewedError ? 'Пять шагов позади; шестой ждёт ваш пример ошибки.' : stage === 0 ? 'На сегодня по этому слову всё.' : `Шаг ${stage} сохранён. Продолжение — завтра.`
  const detail = guided
    ? 'Была подсказка или опора, поэтому попытка не идёт в зачёт — это нормально, тренировка всё равно полезна. Самостоятельная попытка вернётся в другой день.'
    : stage >= 8
    ? 'Вы прошли весь цикл: узнавание, воспроизведение, устную попытку и отложенное извлечение. Слово считается активным.'
    : stage >= 7
      ? 'Устная попытка сохранена. Финальное повторение откроется не раньше чем через 12 часов — так проверяется, что слово удержалось, а не «запомнилось до вечера».'
      : lacksReviewedError
        ? 'Шестой шаг — исправить СВОЮ типичную ошибку с этим словом. Пока вашего примера нет, шаг закрыт: случайная опечатка вместо настоящей ошибки ничего не тренирует. Добавьте пример в «Лаборатории ошибок» — и шаг откроется.'
        : stage === 6
          ? 'Устную попытку лучше сделать позже, когда форма не перед глазами, — иначе это чтение вслух, а не речь. Сейчас доступна только репетиция с опорой; она не засчитывается.'
          : stage === 0
            ? 'Сегодняшняя порция шагов по новым словам уже израсходована. Это не сбой: одно слово — один самостоятельный шаг в день, так оно уходит в долгую память, а не забывается к вечеру. Завтра слово само появится в занятии.'
            : 'Пауза между шагами — не ограничение, а метод: именно она делает запоминание прочным. Завтра следующий шаг появится в обычном занятии.'
  return (
      ...
      <Card level="leaf">
        <CardBody className="empty-state py-14">
          ...
          <div className="flex flex-wrap justify-center gap-2">
            {!guided && lacksReviewedError && <Link to="/errors" className={buttonClass()}>Открыть Лабораторию ошибок</Link>}
