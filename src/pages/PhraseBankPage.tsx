        ...
        {selected ? <aside aria-label="Детали выражения" className="xl:sticky xl:top-[104px] xl:self-start"><Card level="leaf">
          ...
          {editing ? <EditPhraseForm phrase={selected} onSave={(patch) => { const result = updatePhrase(selected.id, patch); if (result.id) setEditing(false); return result }} onCancel={() => setEditing(false)} /> : <CardBody className="space-y-6">
            ...
            {selectedEvidence && <section><p className="inspector-label">Основание статуса</p><Card level="recess" className="mt-3 space-y-3 p-4">
              <EvidenceRule label="Отработка из 8 шагов" value={Math.min(8, selected.activationStage ?? 0)} max={8} />
              <EvidenceRule label="Употребления без подсказки" value={Math.min(2, selectedEvidence.productivePasses)} max={2} />
              <EvidenceFlag label="Использование в Mission" value={selectedEvidence.missionCompletedAt !== undefined ? 'засчитано как подтверждение' : 'необязательно'} done={selectedEvidence.missionCompletedAt !== undefined} />
              <EvidenceFlag label="Вспомнили через 30+ дней" value={selectedEvidence.retained ? 'подтверждено' : 'ещё нет'} done={selectedEvidence.retained} />
            </Card></section>}
