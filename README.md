...
- **Session — cards as the core.** The primary daily surface is now a simple card trainer: see a word, pick one of three answers, read the translation and example, and move on at your own pace (button or Enter). Decks mix today's new words with due reviews; directions are EN→RU, RU→EN, or shuffled; extra rounds are pure training and never distort the schedule. The deep eight-step cycle remains as Advanced practice.
- **Listening you control.** The daily dictation shows a real verdict (words-in-place score plus a word-level diff of the reference), and you choose the load: repeat the same phrase or take a new one.
- **macOS recording fixed.** Takes are captured as mp4 first — the desktop WebView cannot play webm back — and a silent capture now explains the microphone-permission fix instead of playing nothing.
- **Plain language everywhere.** Developer terms are gone from the interface; dead-end screens explain the why; new words per day is configurable (1–30).
- **Distribution.** Universal macOS DMG (Apple Silicon + Intel) and a Windows NSIS installer are published to GitHub Releases on every version tag.
...
- **Simpler navigation:** Today, Words & phrases, Grammar, Session, Listen & speak, and Progress form the primary learning route.
